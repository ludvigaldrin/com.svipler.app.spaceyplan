module.exports = {
    ...
    async get({ homey, deviceId }) {
        try {
            ...
            // Homey usually reports the correct unit on the capability itself;
            // prefer that over our static fallback table so custom/less common
            // sensor types (e.g. from community apps) still get the right unit.
            const reportedUnit = device.capabilitiesObj?.[measureType]?.units;

            // Prepare result object
            const result = {
                value: value !== undefined ? value : null,
                measureType: measureType,
                unit: reportedUnit || this.getUnit(measureType)
            };
    ...
    // Used as a fallback when Homey doesn't report a unit on the capability
    // itself (see reportedUnit above).
    getUnit(measureType) {
        switch (measureType) {
