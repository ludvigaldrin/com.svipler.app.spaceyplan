const windowcoveringRenderer = {
    ...
    createDeviceElement(device, position) {
        ...
        if (!document.getElementById('coverModalStyles')) {
            const styles = document.createElement('style');
            styles.id = 'coverModalStyles';
            styles.textContent = `
                .cover-modal-overlay {
                    position: fixed;
                ...
                .cover-modal .cover-position-label {
                    margin: 0 0 16px 0;
                    font-size: 13px;
                    color: #666;
                }
                ...
                .cover-flow-button .material-symbols-outlined {
                    font-size: 18px;
                }
                .cover-flow-button:disabled {
                    opacity: 0.6;
                    cursor: default;
                }
            `;
        ...
        deviceEl.setAttribute('data-position', device.coverPosition ?? 0);
        ...
        positionDevice().catch(error => {
            Homey.api('POST', '/error', { message: `Error positioning cover device: ${JSON.stringify(error)}` });
        });
    ...
    async initializeState(deviceEl, deviceId, widgetId) {
        try {
            const response = await Homey.api('GET', `/devices/${deviceId}/capabilities/windowcovering`);

            if (response !== undefined && response !== null) {
                const cPosition = typeof response.position === 'number' ? response.position : 0;
                const open = cPosition > 0;

                deviceEl.setAttribute('data-state', open);
                deviceEl.setAttribute('data-position', cPosition);
                deviceEl.classList.toggle('on', open);

                const deviceData = JSON.parse(deviceEl.getAttribute('data-device'));
                deviceData.state = open;
                deviceData.coverPosition = cPosition;
            ...
            await Homey.api('POST', `/subscribeToDevices`, {
                widgetId: widgetId,
                devices: [{ deviceId: deviceId, capability: 'windowcoverings_set' }]
            });

        } catch (error) {
            Homey.api('POST', '/error', { message: `Error in windowcovering initializeState: ${JSON.stringify(error)}` });
        }
    },
    ...
    attachIconEvents(icon, deviceEl) {
        ...
        // Slightly more forgiving than the 10px used elsewhere: holding a
        // finger still on a small 28px target for 500ms naturally causes a
        // bit more drift than a quick tap does.
        const TOLERANCE = 18;
        ...
        const start = (clientX, clientY) => {
            ...
            // The device element has a "transition: all 0.3s ease" base style
            // (used for smooth color changes). Left in place, it also delays
            // this press feedback, so the user can't feel that the hold
            // registered. Disable it for the duration of the interaction.
            ...
            longPressTimer = setTimeout(() => {
                if (moved) return;
                longPressFired = true;
                // Brief scale pulse to confirm the hold registered, then
                // open the modal right away — no extra delay/timer here,
                // since a second async step is one more place a stray
                // touchend/touchcancel could leave things half-finished.
                deviceEl.style.transform = this.addScaleTransform(deviceEl, 1.15);
                restoreVisuals();
                this.showPositionModal(deviceEl);
            }, LONG_PRESS_MS);
        };
        ...
        const end = () => {
            ...
            if (longPressFired) {
                // Modal already opened from the timer; nothing left to do.
                return;
            }
        ...
        // If iOS hands the gesture off to a native action (e.g. scrolling),
        // it fires touchcancel instead of touchend. Without handling it the
        // timer/visual state could be left stuck, or fire the long-press
        // late with no finger on the icon anymore.
        const cancel = () => {
            if (longPressTimer) {
                clearTimeout(longPressTimer);
                longPressTimer = null;
    ...
    // Short tap: drive fully open or fully closed, whichever is not the current state
    async handleClick(deviceEl) {
        try {
            ...
            const currentPosition = parseFloat(deviceEl.getAttribute('data-position')) || 0;
            const targetPosition = currentPosition > 0 ? 0 : 1;

            await this.setPosition(deviceEl, deviceId, targetPosition);
        } catch (error) {
            Homey.api('POST', '/error', { message: `Error in windowcovering handleClick: ${JSON.stringify(error)}` });
        }
    },

    async setPosition(deviceEl, deviceId, targetPosition) {
        const previousPosition = parseFloat(deviceEl.getAttribute('data-position')) || 0;
        try {
            // Optimistic UI update
            this.handleDeviceUpdate(deviceEl, targetPosition, 'windowcoverings_set');

            await Homey.api('PUT', `/devices/${deviceId}/capabilities/windowcovering`, {
                value: targetPosition
            });
        } catch (error) {
            // Revert on failure
            this.handleDeviceUpdate(deviceEl, previousPosition, 'windowcoverings_set');
            Homey.api('POST', '/error', { message: `Error setting cover position: ${JSON.stringify(error)}` });
        }
    },

    showPositionModal(deviceEl) {
        if (document.querySelector('.cover-modal-overlay')) return;

        const deviceId = deviceEl.getAttribute('data-homey-id');
        const name = deviceEl.getAttribute('data-name');
        let position = parseFloat(deviceEl.getAttribute('data-position')) || 0;
        const deviceData = JSON.parse(deviceEl.getAttribute('data-device'));
        ...
        modal.innerHTML = `
            <h2>${name}</h2>
            <p class="cover-position-label">${Math.round(position * 100)}% open</p>
            <div class="cover-slider-track">
                <div class="cover-slider-fill"></div>
                <div class="cover-slider-thumb"></div>
            </div>
            <div class="cover-quick-buttons">
                <button type="button" data-target="0">Close</button>
                <button type="button" data-target="1">Open</button>
            </div>
            ${flowRule?.config ? `
            <button type="button" class="cover-flow-button">
                <span class="material-symbols-outlined">play_circle</span>
                ${flowRule.config.buttonLabel || 'Run Flow'}
            </button>
            ` : ''}
            <button type="button" class="cover-modal-close">Done</button>
        `;

        overlay.appendChild(modal);
        document.body.appendChild(overlay);

        const track = modal.querySelector('.cover-slider-track');
        const fill = modal.querySelector('.cover-slider-fill');
        const thumb = modal.querySelector('.cover-slider-thumb');
        const label = modal.querySelector('.cover-position-label');

        const renderSlider = (pos) => {
            const pct = Math.max(0, Math.min(1, pos)) * 100;
            fill.style.width = `${pct}%`;
            thumb.style.left = `${pct}%`;
            label.textContent = `${Math.round(pct)}% open`;
        };
        renderSlider(position);

        // Live preview on the floor plan while dragging (no API calls yet)
        const previewOnMap = (pos) => {
            this.handleDeviceUpdate(deviceEl, pos, 'windowcoverings_set');
        };
        ...
        const onDragMove = (clientX) => {
            position = posFromEvent(clientX);
            renderSlider(position);
            previewOnMap(position);
        };
        ...
        track.addEventListener('mousedown', (e) => {
            ...
            const onUp = async () => {
                ...
                document.removeEventListener('mousemove', onMove);
                document.removeEventListener('mouseup', onUp);
                await this.setPosition(deviceEl, deviceId, position);
            };
            document.addEventListener('mousemove', onMove);
            document.addEventListener('mouseup', onUp);
        });
        ...
        modal.querySelectorAll('.cover-quick-buttons button').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.stopPropagation();
                position = parseFloat(btn.getAttribute('data-target'));
                renderSlider(position);
                await this.setPosition(deviceEl, deviceId, position);
            });
        });

        const flowButton = modal.querySelector('.cover-flow-button');
        if (flowButton && flowRule?.config) {
            flowButton.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    Homey.api('POST', '/error', { message: `Error running windowcovering flow: ${JSON.stringify(error)}` });
                } finally {
        ...
        modal.querySelector('.cover-modal-close').addEventListener('click', (e) => {
            e.stopPropagation();
            close();
        });
    ...
    handleDeviceUpdate(deviceEl, value, capability) {
        try {
            ...
            const cPosition = typeof value === 'number' ? value : (value ? 1 : 0);
            const open = cPosition > 0;
            ...
            if (!allColorRule && onOffColorRule?.config) {
                const currentColor = open ? onOffColorRule.config.cloudColorOn : onOffColorRule.config.cloudColorOff;
                const showCloud = open ? onOffColorRule.config.showCloudOn : onOffColorRule.config.showCloudOff;
                const showIcon = open ? onOffColorRule.config.showIconOn : onOffColorRule.config.showIconOff;
                const iconColor = open ? onOffColorRule.config.iconColorOn : onOffColorRule.config.iconColorOff;
            ...
            // Keep an open modal's slider/label in sync (e.g. after a quick-button tap)
            const modal = document.querySelector('.cover-modal-overlay');
            if (modal) {
                const fill = modal.querySelector('.cover-slider-fill');
                const thumb = modal.querySelector('.cover-slider-thumb');
                const label = modal.querySelector('.cover-position-label');
                if (fill && thumb && label) {
                    const pct = Math.max(0, Math.min(1, cPosition)) * 100;
                    fill.style.width = `${pct}%`;
                    thumb.style.left = `${pct}%`;
                    label.textContent = `${Math.round(pct)}% open`;
                }
            ...
            Homey.api('POST', '/error', { message: `Error in windowcovering handleDeviceUpdate: ${JSON.stringify(error)}` });
        }
    },
    ...
    applyInitialRules(device, deviceEl) {
        try {
            ...
            Homey.api('POST', '/error', { message: `Error in windowcovering applyInitialRules: ${JSON.stringify(error)}` });
        }
    },
...
window.capabilityRenderers.windowcovering = windowcoveringRenderer;
